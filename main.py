...
def _load_all_env_sources():
    """Try every possible source of environment variables."""
    # Sources in priority order (later = lower priority via override=False)
    sources = [
        ".env",
        ".env.runtime",      # Generated by Railway build step
        "/etc/environment",
        "/app/.env",
        "/app/.env.runtime",
        "/secrets/.env",
    ...
    # Read from /proc/1/environ (parent PID's environment)
    try:
...
class Orchestrator:
    ...
    async def run(self):
        ...
        if not token or len(token) < 20 or ":" not in token:
            ...
            logger.error("=" * 55)
            # Keep container alive so Railway Console stays accessible
            retry = 0
            while True:
                await asyncio.sleep(30)
                retry += 1
                # Re-read env files in case Railway writes them after start
                _load_all_env_sources()
                token = os.environ.get("TELEGRAM_BOT_TOKEN","").strip()
                if token and len(token) > 20 and ":" in token:
                    logger.info(f"[Init] Token found after {retry*30}s! Proceeding...")
                    break
                if retry % 2 == 0:
                    logger.warning(
                        f"[Wait] No TELEGRAM_BOT_TOKEN after {retry*30}s. "
                        f"Console cmd: export TELEGRAM_BOT_TOKEN=<token> && python main.py"
                    )
